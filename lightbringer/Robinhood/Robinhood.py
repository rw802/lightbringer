...
class Robinhood:
    ...
    def login(
        ...
        Args:
            ...
            mfa_code (int): level 2 authentication
        ...
        if 'mfa_required' in data.keys():           #pragma: no cover
            mfa_code = getpass.getpass("Auth Code: ")
            payload['mfa_code'] = int(mfa_code)
    ...
    def quote_data(self, symbols=''):
        """fetch stock quote

        Args:
            symbols (str): stock ticker, prompt if blank

        Returns:
            (:obj:`list`): list of JSON contents from `quotes` endpoint

        """
        url = None
        if symbols.find(',') == -1:
            url = str(self.endpoints['quotes']) + str(symbols) + "/"
        else:
            url = str(self.endpoints['quotes']) + "?symbols=" + str(symbols)
        ...
        try:
            ...
            raise NameError('Invalid Symbol: ' + symbols) #TODO: custom exception
        if 'results' in data:
            return data['results']
        return [data]

    def get_quote_list(self, symbols='', attrs=''):
        """Returns multiple stock info and keys from quote_data (prompt if blank)

        Args:
            symbols (str): stock ticker (or tickers separated by a comma)
            , prompt if blank
            attrs (str): key attributes that the function should return
            quotes = {
                       'previous_close', 'has_traded',
                       'last_trade_price', 'last_extended_hours_trade_price',
                       'adjusted_previous_close', 'last_trade_price_source',
                       'updated_at', 'ask_price', 'instrument', 'symbol',
                       'trading_halted', 'ask_size', 'bid_price',
                       'previous_close_date', 'bid_size'
                    }
        ...
        def make_row(data, attrs):
            ret = []
            for each in attrs:
                ret.append(data[each])
            return ret
        #Prompt for stock if not entered
        a = attrs.split(',')
        if not symbols:   #pragma: no cover
            symbols = input("Symbol (comma to divede) : ")
        data = self.quote_data(symbols)
        res = []
        for each in data:
            if each:
                res.append(make_row(each, a))
        return res
    ...
    def print_quote(self, stocks=''):    #pragma: no cover
        """print quote information
        Args:
            stocks (str): a string of symbols split by comma

        Returns:
            None

        """
        data = self.get_quote_list(stocks,'symbol,last_trade_price,bid_size,bid_price,ask_size,ask_price')
        # for item in data:
    ...
    def cancel_order(self,
                     orderid):
        '''
        Cancel an order by given orderid
        :param orderid: returned value upon placing an order successfully
        :return:
        '''
        res = self.session.post(
            self.endpoints['orders']+orderid+'/cancel/'
        )
        res.raise_for_status()
        return res